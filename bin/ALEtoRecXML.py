...
def ConvertRTtoLossIndepVersion(RT , speciesTree = None, keptChildNameSuffix = ".c"):
    ...
    *RECURSIVE*
    ...
    if not speciesTree is None:
        dict={}
        for node_test in speciesTree.iter_descendants("postorder"):
            dict[node_test.name]=getattr(node_test.get_sisters()[0],"name")



    for i, e in reversed(list(enumerate(RT.eventRecs))):
        print("i= ",i," e= ",e," name= ",RT.name)
        if (e.eventCode=="C"):
            while(RT.name.endswith(".c")):
                RT.name=RT.name[:-2]
        if len(e.eventCode)>1 and e.eventCode.endswith("L"):
            species = ""

            if not speciesTree is None:
                if not RT.getEvent(i).species==getattr(speciesTree.get_tree_root(),"name"):
                    species=dict[RT.getEvent(i).species]
                else:
                    species=RT.getEvent(i).species
                #species = RT.getLostSpecies( i , speciesTree)
#
#            if not speciesTree is None:
#                if i!=0:
#                    j=max(li)
#                    temp=RT.getEvent(j).species
#                    speciesIdFeature="name"
#                    lostSpeciesNode=speciesTree.search_nodes(**{speciesIdFeature: temp })[0].get_sisters()[0]
#                    species = getattr(lostSpeciesNode, speciesIdFeature)
#                else:
#                    temp=RT.getEvent(i-1).species
#                    speciesIdFeature="name"
#                    lostSpeciesNode=speciesTree.search_nodes(**{speciesIdFeature: temp })[0].get_sisters()[0]
#                    species = getattr(lostSpeciesNode, speciesIdFeature)
#                    print("e= ",e, " and sister= ",species)
