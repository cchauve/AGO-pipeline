...
from AGO_utils import (
    get_last_line_file, 
    newick_get_leaves, newick_get_children, 
    recPhyloXML_read_events, recPhyloXML_format,
    STATS_genes, STATS_dup, STATS_loss, STATS_keys
)
    ...
    with open(parameters['macse_log_file'], 'w') as log_file, \
        ...
        for idx in range(1,len(active_families)+1):
            ...
            if not (os.path.exists(idx_log_file) and os.path.exists(idx_err_file)):
                ...
                continue
            log_last = get_last_line_file(idx_log_file, msg=EMPTY_FILE)
            err_last = get_last_line_file(idx_err_file, msg=EMPTY_FILE)
...
def reformat_generax(parameters):
    with open(parameters['generax_log_file'], 'r') as log_file:
        ...
        for family_data in families_data:
            ...
            start_id = recPhyloXML_format(
                in_xml_rec_tree, out_xml_rec_tree, start_id=start_id
            )
...
def stats_generax(parameters):
    ...
    for family in active_families:
        ...
        try:
            stats_all = recPhyloXML_read_events(rec_tree)
...
def aux_decostar_parameters(parameters):
    ...
    with open(parameters['decostar_parameters_file'], 'w') as decostar_parameters_file:
        ...
        decostar_parameters_file.write('write.newick=false\n')
...
def decostar_species_map(parameters):
    ...
    '''
    active_leaves_map = newick_get_leaves(parameters['active_species_tree'])
...
def aux_sppdcj_species_trees(parameters):
    ...
    '''
    children_map = newick_get_children(parameters['active_species_tree'])
